...
class ResYnet(nn.Module):
    ...
    def __init__(self, layer, BasicBlock, in_channel=4, width=1.0, BN_enable=True, pretrained=False):
        ...
        self.encoder2 = ResNet(BasicBlock, layer, width=0.5, in_channel=4, unet=True, norm_layer=norm_layer)
...
class ResUnet183(nn.Module):
    def __init__(self, width=1, in_channel=8, feat_dim=128):
        super(ResUnet183, self).__init__()
        self.net = ResUnet([2, 2, 2, 2], BasicBlock, width=width, in_channel=in_channel)
        self.val = nn.Conv2d(feat_dim, feat_dim, kernel_size=1, stride=1)
        self.unt = nn.Sequential(
            nn.Conv2d(feat_dim, 32, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, x1, mode=0):
        out, vec = self.net(x1, mode=1)
        return out, nn.AdaptiveAvgPool2d((1, 1))(vec)
...
class ResFeat(nn.Module):
    ...
    def forward(self, x, mode=0):
        feat = self.encoder(x, mode=mode)
        return feat

class ResFeat2(nn.Module):
    def __init__(self, width=1, in_channel=8, feat_dim=128):
        super(ResFeat2, self).__init__()
        self.net = ResFeat([2, 2, 2, 2], BasicBlock, width=width, in_channel=in_channel)

    def forward(self, x1, mode=0):
        out = self.net(x1, mode=1)
        return out