...
from .ResUnetStd import ResUnet, BasicBlock
...
class ConvLSTMNetwork(torch.nn.Module):
    def __init__(self, img_size_list, input_channels, hidden_channels, ouput_channels, kernel_size, num_layers,
        ...
        self.enocer = ResUnet([2, 2, 2, 2], BasicBlock, width=1, in_channel=8)
        ...
        self.contrast = nn.Sequential(nn.Conv2d(hidden_channels[-1] * 2, ouput_channels, kernel_size=1, stride=1,
                                                padding=0, bias=False))

    def forward(self, input_tensor):
        cls_num = torch.from_numpy(np.array([[0], [1]], dtype=np.float32)).cuda()
        input_tensor = self.enocer(input_tensor)
