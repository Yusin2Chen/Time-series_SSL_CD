...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def forward(self,
                orig_feats: torch.Tensor, orig_feats_pos: torch.Tensor, orig_feats_neg: torch.Tensor,
                orig_code: torch.Tensor, orig_code_pos: torch.Tensor, orig_code_neg: torch.Tensor):
        ...
        perm_ordr = super_perm(orig_feats.shape[0], orig_feats.device)
        ...
        pos_intra_loss, pos_intra_cd = self.helper(
            feats, feats[perm_ordr], code, code[perm_ordr], self.pos_intra_shift)
        pos_inter_loss, pos_inter_cd = self.helper(
            feats, feats_pos[perm_ordr], code, code_pos[perm_ordr], self.pos_inter_shift)


        feats_neg = sample(orig_feats_neg, coords2)
        code_neg = sample(orig_code_neg, coords2)
        neg_inter_loss, neg_inter_cd = self.helper(
            feats, feats_neg[perm_ordr], code, code_neg[perm_ordr], self.neg_inter_shift)
