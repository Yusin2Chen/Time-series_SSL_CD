...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def helper(self, f1, f2, cd, similar=True):
        ...
        if similar:
            ...
            #fv = torch.clone(fd).flatten()
            #fv = fv[fv.argsort()]
            #shift = Percentile()(fv, [30])
            #del fv
            shift = 0.31

        if self.stabalize:
            loss = - cd.squeeze(1).clamp(0, 0.8) * (fd - shift)
        else:
            loss = - cd.squeeze(1).clamp(0) * (fd - shift)
    ...
    def forward(self,
        ...
        pos_inter_loss = pos_inter_loss[pos_inter_loss > 0]
        ...
        neg_inter_loss = neg_inter_loss[neg_inter_loss < 0]
