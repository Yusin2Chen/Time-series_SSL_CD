...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def cal_shift(self, f1, f2, cd, similar=True):
        ...
        if similar:
            ...
            for i in range(fd.shape[0]):
                ...
                shift = Percentile()(fi, [10])
                output[i] = fd[i] - shift
                del fi

        return cd.squeeze(), output
    ...
    def helper(self, f1, f2, cd, similar=True):
        ...
        if similar:
            ...
            fv = torch.clone(fd).flatten()
            fv = fv[fv.argsort()]
            shift = Percentile()(fv, [40])
            del fv
            # shift = 0.31

        return cd.squeeze(), fd - shift
    ...
    def forward(self,
        ...
        #label_pos = sample(orig_label_pos, coords1).squeeze()
        feats = orig_feats
        feats_pos = orig_feats_pos
        code = orig_code

        # pos_inter_loss, pos_c = self.helper(feats, feats_pos, code, similar=True)
        pos_inter_loss, pos_c = self.cal_shift(feats, feats_pos, code, similar=True)
        #pos_inter_loss = -1 * pos_inter_loss[pos_c > 0].mean() + 1 * pos_inter_loss[pos_c < 0].mean()
        pos_inter_loss = 1 * pos_inter_loss[pos_c < 0].mean() #-1 * pos_inter_loss[label_pos == 1].mean()
        ...
        code_neg = nega_code
        # neg_inter_loss, neg_c = self.helper(feats_neg, feats_neg_pos, code_neg, similar=False)
        neg_inter_loss, neg_c = self.cal_shift(feats_neg, feats_neg_pos, code_neg, similar=False)
        #neg_inter_loss = -1 * neg_inter_loss[neg_c > 0].mean() + 1 * neg_inter_loss[neg_c < 0].mean()
        neg_inter_loss = -1 * neg_inter_loss[neg_c > 0].mean()

        return self.pos_inter_weight * pos_inter_loss + self.neg_inter_weight * neg_inter_loss
        #return self.neg_inter_weight * neg_inter_loss
