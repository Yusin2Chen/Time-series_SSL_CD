...
def load_s2sample(sample, use_s2hr, use_s2mr, use_s2lr, unlabeled=False):
    ...
    if unlabeled:
        ...
        return {'image': img, 'label': label, 'id': sample["id"], 'fold': sample['fold']}
...
class ImgPair(data.Dataset):
    ...
    def __init__(self,
        ...
        for folder in sample_dirs:
            asc_names = folder.split('/')[0] + '_thres_*.tif'
            ...
            self.samples.append({'s2': s2_locations, 'ref': asc_labels, 'id': os.path.basename(s2_locations[-1]),
                                 'fold': folder})
