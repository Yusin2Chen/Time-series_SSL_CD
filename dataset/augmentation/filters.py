...
def gaussian(window_size, sigma):
    ...
    x = torch.arange(window_size) - torch.div(window_size, 2, rounding_mode='floor')
    if window_size % 2 == 0:
        x = x + 0.5
...
def get_gaussian_kernel2d(kernel_size, sigma, force_even=True):
    """
...
def _compute_padding(kernel_size):
    ...
    computed = [torch.div(k, 2, rounding_mode='floor') for k in kernel_size]
