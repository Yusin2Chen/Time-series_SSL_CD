...
selected_file = [
            'T1286_2921_13', 'T4421_3800_13', 'T5863_3800_13', 'T6752_3104_13',
            'T1327_3160_13', 'T2345_3680_13', 'T5912_3937_13', 'T6752_3115_13',
            'T1429_3296_13', 'T4780_3377_13', 'T5916_3785_13', 'T6761_3129_13',
            'T1433_3310_13', 'T4791_3920_13', 'T5927_3715_13', 'T6762_3348_13',
            'T1446_2989_13', 'T2569_4513_13', 'T6186_3574_13', 'T6763_3346_13',
            'T1474_3210_13', 'T2697_3715_13', 'T4815_3379_13', 'T6764_3347_13',
            'T1479_3214_13', 'T4816_3380_13', 'T6460_3370_13', 'T6813_3313_13',
            'T1487_3335_13', 'T2979_4481_13', 'T6461_3368_13', 'T6824_4117_13',
            ...
            'T2037_3758_13', 'T3998_3016_13', 'T6678_3579_13', 'T7367_5050_13',
            ...
            'T2284_2983_13', 'T4254_2915_13', 'T6730_3430_13', 'T7394_5018_13']
...
class RandomCrop(object):
    ...
    def __call__(self, sample, unlabeld=True):
        ...
        if unlabeld:
            ...
            image, lc = sample['image'], sample['label']
...
class MTCD(data.Dataset):
    ...
    def __init__(self,
        ...
        # filter pos pair, neg_pair
        self.pos_pair = [pair_pos[i] for i in range(len(pair_pos)) if
                         pair_pos[i][0] in selected_file and pair_pos[i][1] in selected_file]
        self.neg_pair = [pair_neg[i] for i in range(len(pair_neg)) if
                         pair_neg[i][0] in selected_file and pair_neg[i][1] in selected_file]
        '''
        for seasonfolder in [
        ...
        for folder in sample_dirs:
            ...
            self.samples[folder.split('/')[0]] = [s2_locations_asc, asc_labels]
    ...
    def __getitem__(self, index):
        ...
        if index == 0:
            samples_index = sample(range(len(self.neg_pair)), 63)
            samples_pos = [self.pos_pair[i] for i in samples_index]
            samples_neg = [self.neg_pair[i] for i in samples_index]
            self.sample_slf = [{'img': self.samples[i][0], 'ref': self.samples[i][1]} for i in
                               [j[0] for j in samples_pos]]
            self.sample_pos = [{'img': self.samples[i][0], 'ref': self.samples[i][1]} for i in
                               [j[1] for j in samples_pos]]
            self.sample_neg = [{'img': self.samples[i][0], 'ref': self.samples[i][1]} for i in
                               [j[1] for j in samples_neg]]
        # get and load sample from index file
        sample_slf = self.sample_slf[index]
        sample_pos = self.sample_pos[index]
        sample_neg = self.sample_neg[index]
        if self.datatype == 'S2':
            data_sample_a = load_s2sample(sample_slf, self.use_s2hr, self.use_s2mr, self.use_s2lr,
                                          unlabeled=self.unlabeled)
            data_sample_b = load_s2sample(sample_neg, self.use_s2hr, self.use_s2mr, self.use_s2lr,
                                          unlabeled=self.unlabeled)
        elif self.datatype == 'S1':
            data_sample_a = load_s1sample(sample_slf, unlabeled=self.unlabeled)
            data_sample_b = load_s1sample(sample_neg, unlabeled=self.unlabeled)
        elif self.datatype == 'PL':
            data_sample_a = load_plsample(sample_slf, unlabeled=self.unlabeled)
            data_sample_b = load_plsample(sample_neg, unlabeled=self.unlabeled)
        else:
            print('no this data!!!')
            data_sample_a, data_sample_b = None, None
        if self.transform:
            return self.transform(data_sample_a, unlabeld=self.unlabeled), \
                   self.transform(data_sample_b, unlabeld=self.unlabeled)
        else:
            return data_sample_a, data_sample_b
