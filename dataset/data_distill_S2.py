...
class S2(data.Dataset):
    ...
    def __init__(self, path, use_s2hr=True, use_s2mr=False, use_s2lr=False, transform=False,):
        ...
        for folder in sample_dirs:
            ...
            for s2_loc in tqdm(s2_locations, desc="[Load]"):
                s2_pos = s2_loc.replace('s2_', 's2p_').replace('.tif', '.pkl')
                if os.path.exists(s2_pos):
                    with open(s2_pos, 'rb') as fr:
                        data = pickle.load(fr)
                        s2p_loc = sample(data, 1)[0]
                else:
                    continue
                self.sample_slf.append({"s2": s2_loc})
                self.sample_pos.append({"s2p": s2p_loc})

        print("loaded", len(self.sample_slf), "samples from the dataset")
    ...
    def __getitem__(self, index):
        ...
        img_slf = load_s2(sample_slf["s2"], self.use_s2hr, self.use_s2mr, self.use_s2lr)
        img_pos = load_s2(sample_pos["s2p"], self.use_s2hr, self.use_s2mr, self.use_s2lr)
        return {'img_slf': img_slf, 'img_pos': img_pos}


    def __len__(self):
        """Get number of samples in the dataset"""
        return len(self.sample_slf)
