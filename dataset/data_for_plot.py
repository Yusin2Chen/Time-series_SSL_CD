...
def load_s2sample(sample, use_s2hr, use_s2mr, use_s2lr, unlabeled=False):
    ...
    # load label data
    label = load_lc(sample["asc"][0])[np.newaxis, :]
    for i in range(1, len(sample["asc"])):
        label = np.concatenate((label, load_lc(sample["asc"][i])[np.newaxis, :]), axis=0)
    # load label
    thres = load_lc(sample["thres"][0])[np.newaxis, :]
    for i in range(1, len(sample["thres"])):
        thres = np.concatenate((thres, load_lc(sample["thres"][i])[np.newaxis, :]), axis=0)
    # load label
    if unlabeled:
        return {'image': img, 'label': label, 'thres': thres, 'id': sample['id']}
    else:
        return {'image': img, 'label': label, 'thres': thres, 'id': sample['id']}
...
class MTCD(data.Dataset):
    ...
    def __init__(self,
        ...
        for folder in sample_dirs:
            ...
            thres_names = folder.split('/')[0] + '_thres_*.tif'
            thres_labels = glob.glob(os.path.join(path, f"{folder}/{thres_names}"), recursive=True)
            thres_labels = sorted(thres_labels, key=lambda x: int(os.path.basename(x).split('.')[0].split('_')[-1]))
            # ascending
            s2_locations_asc = sorted(s2_locations, key=lambda x: int(os.path.basename(x)[0:6]), reverse=True)
            self.samples.append({'img': s2_locations_asc, 'asc': asc_labels, 'thres': thres_labels, 'id': folder})
