...
class MTCD(data.Dataset):
    ...
    def __init__(self,
        ...
        for folder in sample_dirs:
            ...
            self.samples.append({'img': s2_locations_asc, 'id': folder})
    ...
    def __getitem__(self, index):
        ...
        if index == 0:
            ...
            self.sample_slf = [self.samples[i] for i in [j[0] for j in samples_pos]]
            self.sample_pos = [self.samples[i] for i in [j[1] for j in samples_pos]]
            self.sample_neg = [self.samples[i] for i in [j[1] for j in samples_neg]]
        ...
        if self.datatype == 'S2':
            ...
            data_slf = load_s1sample(sample_slf, unlabeled=self.unlabeled)
            data_pos = load_s1sample(sample_pos, unlabeled=self.unlabeled)
            data_neg = load_s1sample(sample_neg, unlabeled=self.unlabeled)
