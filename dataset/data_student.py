...
def load_s2sample(sample, use_s2hr, use_s2mr, use_s2lr, unlabeled=False):
    ...
    img = np.concatenate((img, load_s2(sample["s2"], use_s2hr, use_s2mr, use_s2lr)), axis=0)
...
def load_s1sample(sample, unlabeled=False):
    ...
    img = np.concatenate((img, load_s1(sample["s2"])), axis=0)
...
def load_plsample(sample, unlabeled=False):
    img = load_pl(sample["s1"])
    img = np.concatenate((img, load_pl(sample["s2"])), axis=0)
    ...
    def __init__(self,
        ...
        for folder in sample_dirs:
            ...
            if len(s2_locations) > 1:
                ...
                # 下面这句用来筛出跨时过长的 <10
                comb_list = [comb_list[i] for i in range(len(s2_locations) - 1)]
                for (s1_loc, s2_loc) in comb_list:
                    self.samples.append({"s1": s1_loc, "s2": s2_loc, "id": os.path.basename(s2_loc)})
