...
class RandomCrop(object):
    ...
    def __call__(self, sample, unlabeld=True):
        ...
        if unlabeld:
            ...
            for i in range(w):
                for j in range(h):
                    pix_t = copy.deepcopy(lc[:, i, j])
                    if (not np.all(pix_t)) and (5 in pix_t):
                        pix_t[pix_t == 1] = 4
                        pix_t[pix_t == 2] = 4
                        pix_t[pix_t == 5] = 4
                        pix_t[pix_t == 7] = 4
...
class MTCD(data.Dataset):
    ...
    def __init__(self,
        ...
        for folder in [
            ...
            s2_locations = glob.glob(os.path.join(path, f"{folder}/Images/20*.tif"), recursive=True)
            s2_labels = glob.glob(os.path.join(path, f"{folder}/Labels/20*.tif"), recursive=True)
            # ascending
            s2_locations_asc = sorted(s2_locations, key=lambda x: int(os.path.basename(x)[0:6]), reverse=True)
            s2_labels_asc = sorted(s2_labels, key=lambda x: int(os.path.basename(x)[0:6]), reverse=True)
            # self.samples.append({'img': s2_locations_asc, 'ref': s2_labels_asc, 'id': folder})
            self.samples[folder.split('/')[0]] = [s2_locations_asc, s2_labels_asc]
    ...
    def __getitem__(self, index):
        ...
        if index == 0:
            ...
            self.sample_slf = [{'img': self.samples[i][0], 'ref': self.samples[i][1]} for i in [j[0] for j in samples_pos]]
            self.sample_pos = [{'img': self.samples[i][0], 'ref': self.samples[i][1]} for i in [j[1] for j in samples_pos]]
            self.sample_neg = [{'img': self.samples[i][0], 'ref': self.samples[i][1]} for i in [j[1] for j in samples_neg]]
