...
def parse_option():
    ...
    parser.add_argument('--model_path', type=str, default='./save_cls_YLSTM_256_thres', help='path to save model')
    parser.add_argument('--save', type=str, default='./save_cls_YLSTM_256_thres', help='path to save linear classifier')
...
def SPC(sup_mapa, sup_mapat, label_a, idx, corr_idx1, crop_size):  # Segmentation Perceptual Consistency:
    ...
    # consitence loss
    pFeature_A = sup_mapa[idx].contiguous().view(2, -1)
    pFeature_B = sup_mapat[corr_idx1].contiguous().view(2, -1)
    seg_A = get_one_hot(label_a[idx].long()).permute(2, 0, 1).contiguous().view(2, -1).cuda()
    seg_B = get_one_hot(label_a[corr_idx1].long()).permute(2, 0, 1).contiguous().view(2, -1).cuda()
...
class BYOLTrainer:
    def __init__(self, args, online_network, target_network, criterion, optimizer, device):
        ...
        self.sup_loss2 = nn.CrossEntropyLoss(weight=torch.tensor([1., 5.])).cuda()
    ...
    def update(self, image, label):
        ...
        g_feata, cls_feata, cls_mapa, sup_mapa = self.online_network(image_a[:-1].cuda(), condn_a.cuda())
        g_featb, cls_featb, cls_mapb, sup_mapb = self.online_network(image_b[:-1].cuda(), condn_b.cuda())
        with torch.no_grad():
            g_featat, cls_featat, cls_mapat, sup_mapat = self.target_network(t_image_a[:-1].cuda(), condn_a.cuda())
            g_featbt, cls_featbt, cls_mapbt, sup_mapbt = self.target_network(t_image_b[:-1].cuda(), condn_b.cuda())
            ...
            idx = random.sample(range(ni - 1), 1)[0]
            ...
            # consitence loss
        # consistent loss
        loss_consistent1, _, _ = SPC(torch.clone(sup_mapa), torch.clone(sup_mapat),
                                     torch.clone(label_a), idx, corr_idx1, (128, 128))
        loss_consistent2, _, _ = SPC(torch.clone(sup_mapb), torch.clone(sup_mapbt),
                                     torch.clone(label_b), idx, corr_idx2, (128, 128))
        # supervised contrastive loss
        feat1 = torch.cat([F.normalize(cls_mapa.permute(0, 2, 3, 1).contiguous().view(-1, 2).unsqueeze(1), dim=-1),
                           F.normalize(cls_mapat.permute(0, 2, 3, 1).contiguous().view(-1, 2).unsqueeze(1), dim=-1)],
                          dim=1)
        feat2 = torch.cat([F.normalize(cls_mapb.permute(0, 2, 3, 1).contiguous().view(-1, 2).unsqueeze(1), dim=-1),
                           F.normalize(cls_mapbt.permute(0, 2, 3, 1).contiguous().view(-1, 2).unsqueeze(1), dim=-1)],
                          dim=1)
        ...
        if msk0.any() or msk1.any():
            ...
            ############################################################################################################
            loss = 1 * self.sup_loss2(sup_mapa, label_a.long().cuda()) + 1 * self.sup_loss2(sup_mapb, label_b.long().cuda()) + \
                   0.001 * self.sup_con(sfeat[sindice, :], slabel[sindice].cuda().long()) + 0.5 * (2 - loss_consistent1 - loss_consistent2)
        else:
            loss = 1 * self.sup_loss2(sup_mapa, label_a.long().cuda()) + 1 * self.sup_loss2(sup_mapb, label_b.long().cuda()) + 0.5 * (2 - loss_consistent1 - loss_consistent2)
