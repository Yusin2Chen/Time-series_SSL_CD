...
def parse_option():
    ...
    # output
    parser.add_argument('--out_dir', type=str, default='./result_only_sigmoid', help='path to save linear classifier')
    parser.add_argument('--score', action='store_true', default=True, help='score prediction results using ground-truth data')
    parser.add_argument('--preview_dir', type=str, default='./preview_only_sigmoid_RGB', help='path to preview dir (default: no previews)')
...
def lstm_factory(model):
    ...
    pretrained_model = os.path.join('./save_cls_YLSTM_256_sigmoid/ResUnet182/BOYL_epoch_199_0.2982243689129481.pth')
        ...
        for idx, (batch) in enumerate(val_loader):
            ...
            for i in range(length):
                ...
                if args.preview_dir is not None:
                    ...
                    plt.axis("off")
                    plt.savefig(os.path.join(args.preview_dir, id), pad_inches=0, dpi=600, transparent=True,
                                bbox_inches='tight')
                    plt.close()
                    '''
                    if args.score:
