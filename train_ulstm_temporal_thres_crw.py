...
from models.UConvLSTM_temp import ConvLSTMNetwork_
...
def parse_option():
    ...
    parser.add_argument('--model_path', type=str, default='./save_cls_YLSTM_256_thres_crw', help='path to save model')
    parser.add_argument('--save', type=str, default='./save_cls_YLSTM_256_thres_crw', help='path to save linear classifier')
...
class BYOLTrainer:
    ...
    def update(self, image, label):
        ...
        feat_1 = torch.cat(
            [F.normalize(feat_contraa.permute(0, 2, 3, 1).contiguous().view(-1, 16).unsqueeze(1), dim=-1),
...
def main():
    ...
    # set the model
    online_network = ConvLSTMNetwork_([[256, 256], ], input_channels=16, ouput_channels=2, hidden_channels=[16, 16],
